...
class plgJ2StorePayment_sagepay3 extends J2StorePaymentPlugin

{/**
	 * @var $_element  string  Should always correspond with the plugin's filename,
	 *                         forcing it to be unique
	 */
    var $_element    = 'payment_sagepay3';
    var $login_id    = '';
    var $tran_key    = '';
    var $_isLog      = false;
    ...
    function _prePayment( $data )
    {
        ...
        //08 takes as 8 and that format is incorrect in Sagepay >> 817 != 0817
        if (strlen($month)==1){
        	$month = "0".$month;
        }
        ...
        foreach ($submitted_values as $key=>$value)
        {
            switch ($key)
            {
                case "cardholder":
                    ...
                    {
                        $object->error = true;
                        $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_HOLDER_NAME_REQUIRED" )."</li>";
                    }
                  break;
               case "cardtype":
                    if (!isset($submitted_values[$key]) || !JString::strlen($submitted_values[$key]))
                    {
                        $object->error = true;
                        $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_TYPE_INVALID" )."</li>";
                    }
                  break;
                case "cardnum":
                    if (!isset($submitted_values[$key]) || !JString::strlen($submitted_values[$key]))
                    {
                        $object->error = true;
                        $object->message .= "<li> ".$submitted_values[$key]." ".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_NUMBER_INVALID" )."</li>";
                    }
                    
                    if (!ctype_digit($submitted_values[$key]))
                    {
                        $object->error = true;
                        $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_NUMBER_INVALID" )."</li>";
                    }
					if (strlen($submitted_values[$key]) < 13)
					{
					    $object->error = true;
					    $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_NUMBER_LENGHT_INVALID" )."</li>";
					}
					if (strlen($submitted_values[$key]) > 20)
					{
					    $object->error = true;
					    $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_NUMBER_LENGHT_INVALID" )."</li>";
					}                    
                 ...
                 case "month":
                    ...
                    {
                        $object->error = true;
                        $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_EXPIRATION_DATE_INVALID" )."</li>";
                    }
                  break;
                case "year":
                    if (!isset($submitted_values[$key]) || !JString::strlen($submitted_values[$key]))
                ...
                case "cardcvv":
                    ...
                    {
                        $object->error = true;
                        $object->message .= "<li>".JText::_( "J2STORE_SAGEPAY3_MESSAGE_CARD_CVV_INVALID" )."</li>";
                    }
                    if (!ctype_digit($submitted_values[$key]))
                    {
                        $object->error = true;
 ...
 function _cardTypesField( $field='cardtype', $default='', $options='' )
    {
    	...
    	foreach($card_types as $type) {
    		$types[] = JHTML::_('select.option', $type, JText::_( "J2STORE_SAGEPAY3_".strtoupper($type) ) );
    	}
    ...
    function _process()
    {
        ...
        if ( ! JRequest::checkToken() ) {
            return $this->_renderHtml( JText::_( 'J2STORE_SAGEPAY3_INVALID_TOKEN' ) );
        }
		$app = JFactory::getApplication();
        ...
        if($order->load( array('order_id'=>$data['order_id']))) {
	        ...
	        //check for exisiting things
	     	if ( empty($order->order_id) ) {
	             $json['error'] = JText::_( 'J2STORE_SAGEPAY3_INVALID_ORDER' );
	        }

	        if ( empty($this->login_id)) {
	            $json['error'] = JText::_( 'J2STORE_SAGEPAY3_MESSAGE_MISSING_LOGIN_ID' );
	        }
	        if ( empty($this->tran_key)) {
	            $json['error'] = JText::_( 'J2STORE_SAGEPAY3_MESSAGE_MISSING_TRANSACTION_KEY' );
	        }
	        if(!$json) {
...
// basket format	4:Pioneer NSDV99 DVD-Surround Sound System:1:424.68:74.32:499.00: 499.00
	...
	$shipping_country = $this->getCountryById($orderinfo->shipping_country_id)->country_isocode_2;
		...
		if ($sagepay_values["DeliveryCountry"]== "") $sagepay_values["DeliveryCountry"] = $sagepay_values["BillingCountry"];
        if ($sagepay_values["DeliveryFirstnames"]== "") $sagepay_values["DeliveryFirstnames"] = $sagepay_values["BillingFirstnames"];
        if ($sagepay_values["DeliverySurname"]== "") $sagepay_values["DeliverySurname"] = $sagepay_values["BillingSurname"];
        if ($sagepay_values["DeliveryAddress1"]== "") $sagepay_values["DeliveryAddress1"] = $sagepay_values["BillingAddress1"];
        if ($sagepay_values["DeliveryCity"]== "") $sagepay_values["DeliveryCity"] = $sagepay_values["BillingCity"];
        if ($sagepay_values["DeliveryPostCode"]== "") $sagepay_values["DeliveryPostCode"] = $sagepay_values["BillingPostCode"];
        //add optional fields
      //  if($this->partner_id) {
   ...
   //voveran
        ...
        // =======================
            ...
            if($orderpayment->load( array('order_id'=>$order_id))) {
	           ...
	           $order->transaction_details  = $transaction_details;
				...
				if(isset($resp['Status'])) {
					switch($resp['Status']) {
						...
						case 'OK':
							...
							$return['success']  = JText::_($this->params->get('statusOK', 'Process executed without error'));
						...
						case 'PENDING':
							$order_state_id = 4;
							$return['error'] = JText::_($this->params->get('statusNOTOK', 'Process executed with errors,a problem occurred at SagePay which
							prevented transaction registration. Contact event administrator.'));
							break;
            	...
            	$errors[] = JText::_('J2STORE_SAGEPAY3_ORDER_ID_MISMATCH');
            }

            if ($resp['Status'] != 'OK')
            {
        		$error = $resp['Status']." >> ".$resp['StatusDetail'];
            	$this->_sendErrorEmails($return['error'], $transaction_details);
    ...
    function _sendErrorEmails($message, $paymentData)
    {
		...
		$mailserror = str_replace(' ', '',$this->params->get('emailError'));
    	$mailerror = explode(",", $mailserror);
		...
		$body= "In ".$siteurl."\n";
		$body.="\nCustomer has read: ".$message;
		$body.="\nStatus\n-------\n".$paymentData;
    	$recipients = $this->_getAdmins();
    	$mailer =JFactory::getMailer();
		
    	$subject = JText::sprintf('J2STORE_SAGEPAY3_EMAIL_PAYMENT_NOT_VALIDATED_SUBJECT', $sitename)." site: ".$sitename;
    	...
    	foreach ($recipients as $recipient)
    	{
    		...
    		$mailer->setSender(array( $mailfrom, $fromname ));
    		foreach ($mailerror as $mailerr)
    		{
    		$mailer->AddBCC($mailerr);
    		}
